...
const ProfileTab = ({ setIsProfileComplete }) => {
  ...
  const fetchProfile = async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.get('/accountant/profile');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.patch('/accountant/profile', formDataToSend, {
        headers: {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstance.patch('/accountant/change-password', passwordData);
      if (response.data.success) {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-semibold text-gray-800">Accountant Profile</h2>
